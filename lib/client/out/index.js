...
Program.prototype.show = function(question, result, end){
  ...
  resultEl.innerHTML = [
    ifSet(result.title,'<h3 class="resultTitle">'+result.title+'</h3>'),
    ifSet(result.image,'<img src="'+result.image+'" width="" height="" alt="" />'),
    ifSet(result.content,'<p class="resultPara">'+result.content+'</p>')
  ].join('');
