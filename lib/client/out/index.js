...
function Program(){
  ...
  document.onkeydown = function(e){
    var _id = setTimeout(function(){ // delay this because mouse event is triggered too !
      // console.log(e);
      self.lastEventType = 'keydown';
      if (type == 2 && e.keyCode == 9 && self.arrowEl){
        self.tabQuestion = null;
        self.onchange2(self.arrowEl);
        self.arrowQuestion = null;
      }
    }, 0);
  }
...
Program.prototype.onchange0 = function(e){
  ...
  this.repopulateSelectedAnswers();

  if (this.lastEventType != 'click') return;
...
Program.prototype.onchange2 = function(e){
  ...
  if (this.lastEventType == 'keydown' && this.tabQuestion == this.arrowQuestion){
    this.arrowEl = e;
    this.arrowQuestion = this.tabQuestion = questionId;
    return;
  }
  ...
  var questionEl = document.getElementById(question._id+'-outlet');
...
Program.prototype.validate0 = function(e){
  ...
  var self = this;
  var buff = {};
  this.userAnswers.forEach(function(answer){
...
Program.prototype.validate2 = function(e){
  ...
  var self = this;
  var range = this.userAnswers.filter(function(answer){
    return !!answer.value;
  }).length;
...
/**
 * Repopulate selected answers.
 */
Program.prototype.repopulateSelectedAnswers = function(){
  ...
  for (var i=0; i<this.inputs.length; i++){
    ...
    if (input.checked){
      var question = this.findQuestion(input.dataset.question);
      var answer = this.findAnswer(question, input.dataset.answer);
...
Program.prototype.show = function(question, result, end){
  ...
  if (this.quiz.type === 1){
    ...
    tweetScore = this.userAnswers.length + ' out of ' + this.quiz.questions[0].answers.length;
  }
  if (this.quiz.type === 2){
    ...
    tweetScore = right.length + ' out of ' + this.quiz.questions[0].answers.length;
  }
  ...
  if (!end){
    ...
    if (dataLayer) dataLayer.push({ event: 'quiz-submit', category: result.content });
