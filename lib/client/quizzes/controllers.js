...
App.QuizzesController = Em.ArrayController.extend({
  ...
  actions: {
    ...
    saveQuiz: function(quiz){
      ...
      function saveQuestions(next, quiz, _quiz){
        ...
        quiz.get('questions').forEach(function(question){
          batch.push(function(done){
            ...
            question.save().then(function(_question){

              var batch = new Batch;
              batch.push(saveAnswers.bind(null, question, _question));
              batch.push(saveResults.bind(null, question, _question));
              batch.end(done);
      ...
      function saveResults(question, _question, next){

        var batch = new Batch;

        question.get('results').forEach(function(result){
          batch.push(function(done){
            result.set('question', _question);
    ...
    /**
      * Add new result to `question`.
      *
      * @param {App.Question} question
      */
    addResult: function(question){
      var result = this.store.createRecord('result', {});
      question.get('results').pushObject(result);
    },


    /**
      * Remove result from `question`.
      *
      * @param {App.Question} question
      * @param {App.Result} result
      */
    removeResult: function(question, result){
      ...
      function next(){
        question.get('results').removeObject(result);
      }
